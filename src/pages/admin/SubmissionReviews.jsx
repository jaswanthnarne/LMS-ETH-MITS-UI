...
function TaskReviewRow({ submission, api, action, isSelected, onSelectToggle }) {
  ...
  const isCloudinary = submission.fileUrl && (submission.fileUrl.startsWith('http://') || submission.fileUrl.startsWith('https://'));
  const fileUrl = isCloudinary 
    ? submission.fileUrl 
    : `${api.getUri ? api.getUri() : 'http://127.0.0.1:5000'}/${submission.fileUrl}`;
    ...
    <div className="flex flex-col lg:flex-row justify-between gap-6 bg-bgPrimary border border-borderCool rounded-xl p-5 hover:border-primary/30 transition-all relative pl-12">
      ...
      <div className="w-full lg:w-[280px] shrink-0 flex flex-col gap-3 pt-4 lg:pt-0 lg:pl-5 border-t lg:border-t-0 lg:border-l border-borderCool/60">
        ...
        <button
          ...
          onClick={() => {
            action(() => api.patch(`/api/tasks/submissions/${submission._id}`, { score: Number(score), status, feedback }), 'Submission graded');
          }}
        >
...
export default function SubmissionReviews({ data, forms, setForm, api, action }) {
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedSubIds(displaySubmissions.map(s => s._id));
    } else {
      setSelectedSubIds([]);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="bg-bgSecondary border border-borderCool rounded-xl p-5 shadow-sm">
        ...
        {displaySubmissions.length > 0 && (
          <div className="bg-bgPrimary border border-borderCool rounded-xl p-4 mb-4 flex flex-wrap items-center justify-between gap-4 text-xs">
            <div className="flex items-center gap-2">
              <input 
                type="checkbox" 
                checked={selectedSubIds.length === displaySubmissions.length && displaySubmissions.length > 0} 
                onChange={handleSelectAll}
                className="w-4 h-4 text-primary border-borderCool rounded focus:ring-primary cursor-pointer"
              />
              <span className="font-semibold text-textPrimary">
                {selectedSubIds.length} of {displaySubmissions.length} selected
              </span>
            </div>
        ...
        <DataList emptyText={`No student task submissions match the '${reviewFilter}' queue.`}>
          <div className="grid grid-cols-1 gap-5">
            {displaySubmissions.map((submission) => (
              <TaskReviewRow
                ...
                api={api}
                action={action}
                isSelected={selectedSubIds.includes(submission._id)}
                onSelectToggle={() => handleSelectOne(submission._id)}
              />
            ))}
          </div>
        </DataList>
